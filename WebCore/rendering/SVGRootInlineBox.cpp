...
static void applyTextAnchorToTextChunk(SVGTextChunk& chunk)
{
    ...
    for (; boxIt != boxEnd; ++boxIt) {
        ...
        InlineBox* parentBox = curBox->parent();
        ASSERT(parentBox && (parentBox->isRootInlineBox() || parentBox->isInlineFlowBox()));
...
void SVGRootInlineBox::paintCharacterRangeForTextBox(RenderObject::PaintInfo& paintInfo, int tx, int ty, InlineTextBox* textBox, const SVGChar& svgChar, const UChar* chars, int length)
{
    ...
    if (paintInfo.phase != PaintPhaseSelection && !isPrinting) {
        ...
        if (haveMarkedText  && !markedTextUsesUnderlines)
            textBox->paintMarkedTextBackground(paintInfo.context, tx, ty, styleToUse, font, markedTextRange->startOffset(exception), markedTextRange->endOffset(exception));
    ...
    TextStyle textStyle(0, svgChar.x, textBox->m_toAdd, textBox->m_reversed, textBox->m_dirOverride || styleToUse->visuallyOrdered());
    ...
    if (paintInfo.phase != PaintPhaseSelection) {
        ...
        for (size_t index = 0; index < numUnderlines; ++index) {
            ...
            if (underline.startOffset <= textBox->end()) {
                ...
                textBox->paintMarkedTextUnderline(paintInfo.context, tx, ty, underline);
