...
JSValue* callEvaluateOnNode(const RefPtr<Node>& node, ExecState* exec)
{
...
List ArgumentListNode::evaluateList(ExecState *exec)
{
    ...
    for (ArgumentListNode *n = this; n; n = n->next.get()) {
        JSValue *v = callEvaluateOnNode(n->expr, exec);
...
Completion VarStatementNode::execute(ExecState *exec)
{
    ...
    (void) callEvaluateOnNode(next, exec);
    KJS_CHECKEXCEPTION();
...
Completion ExprStatementNode::execute(ExecState *exec)
{
    ...
    JSValue* v = callEvaluateOnNode(expr, exec);
    KJS_CHECKEXCEPTION();
...
Completion DoWhileNode::execute(ExecState *exec)
{
    ...
    do {
        ...
        bv = callEvaluateOnNode(expr, exec);
        KJS_CHECKEXCEPTION();
...
Completion ForNode::execute(ExecState *exec)
{
    ...
    if (expr1) {
        v = callEvaluateOnNode(expr1, exec);
        KJS_CHECKEXCEPTION();
    }
    while (1) {
        if (expr2) {
            v = callEvaluateOnNode(expr2, exec);
            KJS_CHECKEXCEPTION();
            if (!v->toBoolean(exec))
        ...
        if (expr3) {
            v = callEvaluateOnNode(expr3, exec);
            KJS_CHECKEXCEPTION();
        }
    }
...
Completion ForInNode::execute(ExecState *exec)
{
    ...
    if (varDecl) {
        (void) callEvaluateOnNode(varDecl, exec);
        KJS_CHECKEXCEPTION();
    }
    
    e = callEvaluateOnNode(expr, exec);
    KJS_CHECKEXCEPTION();
    ...
    while (propIt != propList.end()) {
        ...
        if (lexpr->isResolveNode()) {
            ...
            JSValue *v = callEvaluateOnNode(static_cast<DotAccessorNode *>(lexpr.get())->base(), exec);
            KJS_CHECKEXCEPTION();
            ...
            JSValue *v = callEvaluateOnNode(static_cast<BracketAccessorNode *>(lexpr.get())->base(), exec);
            KJS_CHECKEXCEPTION();
            JSValue *v2 = callEvaluateOnNode(static_cast<BracketAccessorNode *>(lexpr.get())->subscript(), exec);
            KJS_CHECKEXCEPTION();
            JSObject *o = v->toObject(exec);
...
Completion ReturnNode::execute(ExecState *exec)
{
    ...
    JSValue *v = callEvaluateOnNode(value, exec);
    KJS_CHECKEXCEPTION();
