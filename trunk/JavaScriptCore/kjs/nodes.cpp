...
// FIXME: evalBlock will need to be moved into the interpreter loop!
extern JSValue* callEvaluateOnNode(const RefPtr<Node>&, ExecState*);
...
Completion CaseBlockNode::evalBlock(ExecState *exec, JSValue *input)
{
  ...
  CaseClauseNode *clause;
    ...
    while (a) {
      clause = a->getClause();
      a = a->getNext();
      v = callEvaluateOnNode(clause, exec);
      KJS_CHECKEXCEPTION
