...
static CollectorBlock* allocateBlock()
{
#if PLATFORM(DARWIN)    
    vm_address_t ptr = 0;
    vm_map(current_task(), &ptr, BLOCK_SIZE, BLOCK_OFFSET_MASK, VM_FLAGS_ANYWHERE, MEMORY_OBJECT_NULL, 0, FALSE, VM_PROT_DEFAULT, VM_PROT_DEFAULT, VM_INHERIT_DEFAULT);
#elif PLATFORM(WIN)
    ...
    size_t adjust = 0;
    if ((ptr & BLOCK_OFFSET_MASK) != 0)
        adjust = BLOCK_SIZE - (ptr & BLOCK_OFFSET_MASK);
